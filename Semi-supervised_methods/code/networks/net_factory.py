...
# parser.add_argument(
#     '--cfg', type=str, default="", help='path to config file', )
# parser.add_argument(
#     "--opts",
...
def net_factory(net_type="unet", in_chns=3, class_num=3):
    if net_type == "unet":
        net = smp.Unet('resnet34', classes=class_num).cuda()
    elif net_type == 'fpn':
        net = smp.FPN('resnet34', classes=class_num).cuda()
    elif net_type == 'pspnet':
        net = smp.PSPNet('resnet34', classes=class_num).cuda()
    elif net_type == "enet":
