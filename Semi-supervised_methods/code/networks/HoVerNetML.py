...
class HoVerNetML(nn.Module):
    ...
    def __init__(self, num_class=None):
        ...
        # If num_class is none, then we are in nucleus detection, not classification, so we don't use this branch
        if self.num_class is not None:
            self.nc_branch = _HoverNetDecoder()
            # classification head
            self.nc_head = nn.Sequential(
                # one channel in output for each class
                nn.Conv2d(in_channels=64, out_channels=self.num_class, kernel_size=1)
            )
...
def loss_hovernet(outputs, ground_truth, num_class=None):
    """
...
def post_process_batch_hovernet(
    outputs, num_class, small_obj_size_thresh=10, kernel_size=21, h=0.5, k=0.5
):
    """
    ...
    if num_class is None:
        ...
        assert len(outputs) == 3, (
            f"num_class={num_class} but outputs has {len(outputs)} elements. Expecting a list "
...
def _vis_outputs_single(
    images, preds, num_class, index=0, ax=None, markersize=5, palette=None
):
    """
    ...
    if num_class is not None:
        ...
        assert (
            len(palette) >= num_class
        ), f"len(palette)={len(palette)} < num_class={num_class}."
    else:
        classification = False
    ...
    assert len(preds.shape) in [
        3,
        4,
    ], f"Preds shape is {preds.shape}. Must be (B, H, W) or (B, num_class, H, W)"
