...
class DDPM(pl.LightningModule):
    ...
    # ##################################
    # ## https://github.com/ExplainableML/UncerGuidedI2I/blob/main/src/losses.py
    # def bayeGen_loss(self, target, out_mean, out_1alpha, out_beta):
    ...
    def get_loss(self, pred, target, mean=True):
        if self.loss_type == 'l1':
            ...
            if mean:
                ...
                # loss = torch.nn.functional.mse_loss(target, pred['out'], reduction='none')
                # loss_baye = self.bayeGen_loss(target, pred['out'], pred['alpha'], pred['beta']) * (current_epoch+1) / 100
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x) #torch.Size([20, 3, 64, 64])
        z = self.get_first_stage_encoding(encoder_posterior).detach() #torch.Size([20, 3, 64, 64])
        ...
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(z) #reconstruction
            out.extend([x, xrec])
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond) #torch.Size([20, 3, 64, 64])
        ...
        # loss_simple, loss_baye = self.get_loss(model_output, target, mean=False)[0].mean(dim=[1, 2, 3]), self.get_loss(model_output, target, mean=False)[1].mean(dim=[1, 2, 3])
        ############################
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   ...
                   plot_diffusion_rows=True, **kwargs):   #这里的n参数代表的图像数
