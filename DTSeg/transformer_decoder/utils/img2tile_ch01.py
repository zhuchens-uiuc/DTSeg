...
class HuBMAPDataset(Dataset):
    def __init__(self, mode, filename, config):
        ...
        self.data = ((self.data+1)*127.5).astype(np.uint8)
...
class HuBMAPDatasetMask(Dataset):
    def __init__(self, mode, filename, config):
        super().__init__()
        self.path = filename
        state = mode.split('/')[0]
...
def make_parse():
    ...
    parser.add_argument('--INPUT_PATH', default='/data114_3/shaozc/Dataset/natcom/via5/', type=str) #(832, 832)
...
if __name__ == '__main__':
    args = make_parse()
    train_val_filename = glob.glob(opj(args.INPUT_PATH, 'images', '*.tif'))
    test_filename = glob.glob(opj(args.INPUT_PATH, 'masks', '*.tif'))
    ...
    for filename in tqdm(test_filename):
        test_dataset = HuBMAPDatasetMask(mode = 'masks', filename = filename, config = args)
