...
def make_parse():
    ...
    parser.add_argument('--INPUT_PATH', default='/data114_3/shaozc/Dataset/BCI_dataset/HE/', type=str) #(887, 878)
    parser.add_argument('--tile_size', default = 256, type=int)
    parser.add_argument('--predict_size', default = 128, type=int) #overlap
...
if __name__ == '__main__':
    args = make_parse()
    train_val_filename = glob.glob(opj(args.INPUT_PATH, 'train', '*.png'))
    test_filename = glob.glob(opj(args.INPUT_PATH, 'test', '*.png'))
    ...
    for filename in tqdm(train_val_filename):
        ...
        train_val_dataset = HuBMAPDataset(mode = 'train', filename = filename, config = args)
    ...
    for filename in tqdm(test_filename):
        test_dataset = HuBMAPDataset(mode = 'test', filename = filename, config = args)
