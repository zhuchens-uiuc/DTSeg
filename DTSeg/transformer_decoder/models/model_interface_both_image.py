...
#---->diffusion
import sys
sys.path.append("/data111/shaozc/SegDiff/latent-diffusion-main/")
...
# steps = [50,150,250]
# blocks = [5,6,7,8,12]
# model_path = '/data111/shaozc/latent-diffusion-main/logs/2023-02-06T21-52-07_livecell256/checkpoints/epoch=000268.ckpt'
# model_config = '/data111/shaozc/latent-diffusion-main/configs/latent-diffusion/livecell256.yaml'
...
class ModelInterfaceBothImage(pl.LightningModule):

    #---->init
    def __init__(self, model, model_feature, model_image, loss, optimizer, **kargs):
        super(ModelInterfaceBothImage, self).__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            results_feature = self.model_feature(batch['image_data'])['logits']
        ...
        # with torch.no_grad():  
        #     self.model_image.eval()      
        #     unlabeled_results_image_dict = self.model_image(batch['unlabeled_image_data'])
        #     unlabeled_logits_image = unlabeled_results_image_dict['logits']
        results_dict = self.model(results_feature, results_image)
        logits = results_dict['logits']
        loss = self.loss(logits.float(), batch['label'].type(torch.int64))
            #  + 0.5 * self.loss(unlabeled_logits_image.float(), unlabeled_label.type(torch.int64))
        # self.log('image',loss_image, prog_bar=True)
        # self.log('feature',loss_feature, prog_bar=True)
        # self.log('mutual',loss_mutual, prog_bar=True)
