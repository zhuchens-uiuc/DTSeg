...
class FeaturetsbaseData(data.Dataset):
...
#https://albumentations.ai/docs/examples/example_kaggle_salt/
        ...
        if state == 'train':
            ...
            self.unlabeled_data = self.data[int(len(self.data)*self.dataset_cfg.label_ratio):].reset_index(drop=True) ###unlabeled
            self.label = self.slide_data.loc[:, 'train_mask'].dropna()

            if 'pseudo_unlabel' in self.label.tolist()[-1]: # retrain
                self.data = self.data
            ...
            self.preprocessor = A.Compose([
                    ...
                    ToTensorV2()])
            self.aug = self._transform_(state)
        ...
        if state == 'pseudo':
            self.train_data = self.slide_data.loc[:, 'train_image'].dropna()
            # self.val_data = self.slide_data.loc[:, 'val_image'].dropna()
            self.data = self.train_data[int(len(self.train_data)*self.dataset_cfg.label_ratio):].tolist()# + self.val_data[int(len(self.val_data)*self.dataset_cfg.label_ratio):].tolist()
            # self.data.reset_index(inplace=True, drop=True)
            self.train_label = self.slide_data.loc[:, 'train_mask'].dropna()
            # self.val_label = self.slide_data.loc[:, 'val_mask'].dropna()
            self.label = self.train_label[int(len(self.train_label)*self.dataset_cfg.label_ratio):].tolist()# + self.val_label[int(len(self.val_label)*self.dataset_cfg.label_ratio):].tolist()
            # self.label.reset_index(inplace=True, drop=True)
            self.preprocessor = A.Compose([A.Resize(height=self.image_size,width=self.image_size), 
                                                        ToTensorV2()]) #暂时简单处理一下
            self.aug = self._transform_(state)
    ...
    def __getitem__(self, idx):
        ...
        if self.state=='train':
            unlabeled_image_path = np.random.choice(self.unlabeled_data.tolist(), 1)[0]
            # print(unlabeled_image_path)
            unlabeled_tensor_image = self.preprocess_image(unlabeled_image_path)

        label_path = self.label[idx]
        label = self.load_ann(label_path).astype('uint8')
        label = cv2.resize(
        ...
        ######## aug
        transformerd_data = self.aug({'img': image_path, 'label':label_path})
        tensor_image_aug = transformerd_data['img']
        tensor_label_aug = transformerd_data['label'].squeeze()
        # tensor_label_aug[(tensor_label_aug == 3) | (tensor_label_aug == 4)] = 3
        # tensor_label_aug[(tensor_label_aug == 5) | (tensor_label_aug == 6) | (tensor_label_aug == 7)] = 4

        example['data'] = tensor_image_aug #augmentation
        example['image_data'] = tensor_image_aug
        if self.state=='train':
            # example['unlabeled_data'] = unlabeled_tensor_image
            

            ######## aug
            unlabeled_transformerd_data = self.aug({'img': unlabeled_image_path, 'label':label_path})
            unlabeled_tensor_image = unlabeled_transformerd_data['img']
            example['unlabeled_data'] = unlabeled_tensor_image #augmentation
            example['unlabeled_image_data'] = unlabeled_tensor_image
        example['label'] = tensor_label
        example['aug_label'] = tensor_label_aug
        ...
        if state == 'train':
            train_transforms = Compose(
                [
                    ...
                    # # intensity transform
                    RandGaussianNoised(keys=["img"], prob=0.25, mean=0, std=0.1), #0.25
